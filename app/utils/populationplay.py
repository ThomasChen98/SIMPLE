...
from utils.files import load_model_with_id, load_all_models, get_best_model_name, get_model_length, get_opponent_length, get_current_opponent_name_id
        ...
        def setup_opponents(self):
            ...
            if get_model_length(self.name, self.threadID) > self.model_length:
                ...
                if get_opponent_length(self.name, self.threadID) == self.opponent_length: # get new opponent id
                    ...
                    best_model_name = get_best_model_name(self.name, self.opponent_id)

                    logger.info(f'+++Thread {self.threadID}+++ New opponent model: {best_model_name}, previous opponent model = {self.best_model_name}')
                    self.opponent_model = load_model_with_id(self, best_model_name, self.opponent_id)
