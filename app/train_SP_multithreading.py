# sudo docker-compose exec app mpirun -np 2 python3 train_SP_multithreading.py -r -e tictactoe -tt 1e6 -tn 2
...
def main(threadID, args):
    ...
    logger.info(f'\n+++Thread {threadID} Rank {rank}+++ Setting up the selfplay training environment opponents...')
    base_env = get_environment(args.env_name)
    env = selfplay_wrapper(base_env)(threadID = threadID, opponent_type = args.opponent_type, verbose = args.verbose)
    env.seed(workerseed)
    ...
    #Callbacks
    logger.info(f'\n+++Thread {threadID} Rank {rank}+++ Setting up the selfplay evaluation environment opponents...')
    callback_args = {
        'eval_env': selfplay_wrapper(base_env)(threadID=threadID, opponent_type = args.opponent_type, verbose = args.verbose),
    ...
    eval_callback = SelfPlayCallback(threadID, args.opponent_type, args.threshold, args.env_name, **callback_args)
...
if __name__ =="__main__":
    ...
    # creating thread
    thread_list =  [threading.Thread(target=main, name='th_'+str(i), args=(i, args)) for i in range(args.thread_num)]
